...
class UnetFrontend(nn.Module):
    """
    A simple Unet-encoder. Last layer feature map will become
    transformer's input at timestep t.
...
class HiveFormerVisionFrontend(nn.Module):
    ...
    max_horizons:
        length of demo
    max_patches:
        number of pixels (1x1 patch) we have on feature map
    """
